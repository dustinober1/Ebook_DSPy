...
- [ ] **1.2.1** Create the planning directory:
  ```bash
  mkdir -p planning
  ```

- [ ] **1.2.2** Move administrative files to `planning/`:
...
- [ ] **1.2.3** Create GitHub Issues from planning documents (recommended for future):
  - Document a note in `planning/README.md` explaining these files should become GitHub Issues
  - Example content for `planning/README.md`:
    ```markdown
    # Planning Documents (Archive)
    
    These documents contain planning notes and reviews that should eventually 
    be migrated to GitHub Issues and Projects for better tracking.
    
    ## Files in this directory:
    - `uiux.md` - UI/UX design notes
    - `update_plan.md` - Repository refactoring plan
    ```

- [ ] **1.2.4** Remove `index.html` from root (will be auto-generated by Jupyter Book):
